...
/obj/effect/proc_holder/spell/proc/shadowling_check(mob/living/carbon/human/user)
	...
	if(isshadowling(user) && user.mind.has_antag_datum(/datum/antagonist/shadowling))
		return TRUE

	//if(isshadowlinglesser(user) && user.mind.has_antag_datum(/datum/antagonist/shadowling/thrall))
	//	return TRUE
 ...
 */
/obj/effect/proc_holder/spell/pointed/shadowling_glare
	name = "Glare"
	...
	active_msg = "<span class='notice'>Your prepare to your eyes for a stunning glare! <B>Left-click to cast at a target!</B></span>"
	deactive_msg = "<span class='notice'>Your eyes relax... for now.</span>"

/obj/effect/proc_holder/spell/pointed/shadowling_glare/can_cast(mob/living/user = usr, charge_check = TRUE, show_message = FALSE)
	if(user.incorporeal_move == INCORPOREAL_MOVE_BASIC)
		return FALSE
	. = ..()

/obj/effect/proc_holder/spell/pointed/shadowling_glare/can_target(atom/target, mob/user, silent)
	. = ..()
	if(!.)
		return FALSE
	if(!isliving(target))
		...
		return FALSE
	var/mob/M = target
	return !M.stat && !is_shadow_or_thrall(M)

/obj/effect/proc_holder/spell/pointed/shadowling_glare/cast(list/targets, mob/user = usr)
	...
	if(distance <= 2)
		...
		target.AdjustKnockdown(4 SECONDS)
		target.AdjustSilence(20 SECONDS)
		target.adjustStaminaLoss(20)
		target.apply_status_effect(STATUS_EFFECT_STAGGERED, 5 SECONDS)

	else //Distant glare
		// TODO: auto dir to target lol
...
/obj/effect/proc_holder/spell/aoe_turf/shadowling_veil
	name = "Veil"
	desc = "Extinguishes most nearby light sources."
	panel = "Shadowling Abilities"
	cooldown_min = 15 SECONDS //Short cooldown because people can just turn the lights back on
	clothes_req = FALSE
...
/obj/effect/proc_holder/spell/aoe_turf/shadowling_veil/can_cast(mob/living/user = usr, charge_check = TRUE, show_message = FALSE)
	if(user.incorporeal_move == INCORPOREAL_MOVE_BASIC)
		return FALSE
	. = ..()

/obj/effect/proc_holder/spell/aoe_turf/shadowling_veil/cast(list/targets, mob/user = usr)
	if(!shadowling_check(user))
		revert_cast(user)
		return

	to_chat(user, "<span class='shadowling'>You silently disable all nearby lights.</span>")
	//Quite rudimentary if you count that TG has no extinguish_light() proc like bay for every single light-emitting atom
...
/obj/effect/proc_holder/spell/self/shadowling_shadow_walk
	name = "Shadow Walk"
	desc = "Phases you into the space between worlds for a short time, allowing movement through walls and invisbility."
	panel = "Shadowling Abilities"
	cooldown_min = 30 SECONDS //Used to be twice this, buffed
	clothes_req = FALSE
	phase_allowed = TRUE
	action_icon = 'icons/mob/actions/actions_shadowling.dmi'
	action_icon_state = "shadow_walk"

/obj/effect/proc_holder/spell/self/shadowling_shadow_walk/cast(list/targets, mob/living/user = usr)
	if(!shadowling_check(user))
		revert_cast(user)
		return
	...
	sleep(4 SECONDS) // TODO: monometr 4 secundi sound
	if(QDELETED(user))
		return
	...
	user.float(FALSE, TRUE)
	user.incorporeal_move = FALSE
	user.alpha = 255
	user.forceMove(get_turf(user))


/obj/effect/proc_holder/spell/self/shadowling_guise
	name = "Guise"
...
/obj/effect/proc_holder/spell/self/shadowling_guise/cast(list/targets, mob/user = usr)
...
/obj/effect/proc_holder/spell/self/shadowling_guise/proc/reveal(mob/user)
	if(QDELETED(user))
		return
...
*/
/obj/effect/proc_holder/spell/self/shadowling_vision
...
/obj/effect/proc_holder/spell/self/shadowling_vision/cast(list/targets, mob/user = usr)
	var/mob/living/carbon/human/H = user
	if(HAS_TRAIT(H, TRAIT_TRUE_NIGHT_VISION))
		REMOVE_TRAIT(H, TRAIT_TRUE_NIGHT_VISION, TRAIT_GENERIC)
	else
		ADD_TRAIT(H, TRAIT_TRUE_NIGHT_VISION, TRAIT_GENERIC)
	user.update_sight()

/obj/effect/proc_holder/spell/self/shadowling_vision/thrall
	desc = "Thrall Darksight"
	desc = "Gives you night vision."
	panel = "Thrall Abilities"

/obj/effect/proc_holder/spell/targeted/shadowling_icy_veins
	name = "Icy Veins"
...
/obj/effect/proc_holder/spell/targeted/shadowling_icy_veins/can_cast(mob/living/user = usr, charge_check = TRUE, show_message = FALSE)
	if(user.incorporeal_move == INCORPOREAL_MOVE_BASIC)
		return FALSE
	. = ..()

/obj/effect/proc_holder/spell/targeted/shadowling_icy_veins/cast(list/targets, mob/user = usr)
	if(!shadowling_check(user))
		revert_cast(user)
		return
...
/**
 * Turns a target into the shadowling's slave.
 * This overrides all previous loyalties
 */
/obj/effect/proc_holder/spell/pointed/shadowling_enthrall
	name = "Enthrall"
	...
	active_msg = "<span class='notice'>Your prepare your mind to entrall a mortal. <B>Left-click to cast at a target!</B></span>"
	deactive_msg= "<span class='notice'>Your mind relaxes.</span>"
	//need_active_overlay = TRUE
	var/enthralling = FALSE

/obj/effect/proc_holder/spell/pointed/shadowling_enthrall/can_cast(mob/living/user = usr, charge_check = TRUE, show_message = FALSE)
	if(enthralling || user.incorporeal_move == INCORPOREAL_MOVE_BASIC)
		return FALSE
	. = ..()

/obj/effect/proc_holder/spell/pointed/shadowling_enthrall/can_target(atom/target, mob/user, silent)
	. = ..()
	if(!ishuman(target))
		if(!silent)
			to_chat(user, "<span class='warning'>You can't enthrall [target]!</span>")
		return FALSE
	var/mob/living/M = target
	return M.key && M.mind && !M.stat && !is_shadow_or_thrall(M) && M.client

/obj/effect/proc_holder/spell/pointed/shadowling_enthrall/cast(list/targets, mob/user = usr)
	...
	var/datum/antagonist/shadowling/D = user.mind.has_antag_datum(/datum/antagonist/shadowling)
	D.ling_team.add_thrall(target.mind, D.ling_team)
	//target.mind.special_role = ROLE_SHADOWLING_THRALL


/**
 * Resets a shadowling's species to normal, removes genetic defects, and re-equips their armor.
 */
/obj/effect/proc_holder/spell/self/shadowling_regen_armor
	name = "Rapid Re-Hatch"
...
/obj/effect/proc_holder/spell/self/shadowling_regen_armor/cast(list/targets, mob/living/carbon/human/user = usr)
	if(!is_shadow(user))
	...
	user.set_species(/datum/species/shadowling)
 ...
 */
/obj/effect/proc_holder/spell/self/shadowling_collective_mind
	...
	panel = "Shadowling Abilities"
	cooldown_min = 30 SECONDS //30 second cooldown to prevent spam
	clothes_req = FALSE
...
/obj/effect/proc_holder/spell/self/shadowling_collective_mind/can_cast(mob/living/user = usr, charge_check = TRUE, show_message = FALSE)
	if(user.incorporeal_move == INCORPOREAL_MOVE_BASIC)
		return FALSE
	. = ..()


/obj/effect/proc_holder/spell/self/shadowling_collective_mind/cast(list/targets, mob/user = usr)
	if(!shadowling_check(user))
		revert_cast(user)
		return
	...
	var/thralls = 0
	var/datum/antagonist/shadowling/D = user.mind.has_antag_datum(/datum/antagonist/shadowling)
	var/victory_threshold = D.ling_team.required_thralls

	for(var/datum/mind/member in D.ling_team.members)
		if(member.has_antag_datum(/datum/antagonist/shadowling/thrall))
			thralls++
			to_chat(member.current, "<span class='shadowling'>You feel hooks sink into your mind and pull.</span>")
	...
	if(thralls >= CEILING(3 * D.ling_team.thrall_ratio, 1) && !screech_acquired)
		...
		user.mind.AddSpell(new /obj/effect/proc_holder/spell/aoe_turf/shadowling_screech(null))

	if(thralls >= CEILING(5 * D.ling_team.thrall_ratio, 1) && !blind_smoke_acquired)
		...
		user.mind.AddSpell(new /obj/effect/proc_holder/spell/self/shadowling_blindness_smoke(null))

	if(thralls >= CEILING(7 * D.ling_team.thrall_ratio, 1) && !null_charge_acquired)
		...
		user.mind.AddSpell(new /obj/effect/proc_holder/spell/pointed/shadowling_null_charge(null))

	if(thralls >= CEILING(9 * D.ling_team.thrall_ratio, 1) && !revive_thrall_acquired)
		...
		user.mind.AddSpell(new /obj/effect/proc_holder/spell/pointed/shadowling_revive_thrall(null))
	...
	if(thralls < victory_threshold)
		...
		for(var/datum/mind/M in D.ling_team.members)
			if(!is_shadow(M.current))
				continue

			var/obj/effect/proc_holder/spell/self/shadowling_collective_mind/cm_spell = locate() in M.spell_list
			if(cm_spell)
				M.spell_list -= cm_spell
				qdel(cm_spell)

			M.RemoveSpell(/obj/effect/proc_holder/spell/self/shadowling_hatch)
			M.AddSpell(new /obj/effect/proc_holder/spell/self/shadowling_ascend(null))

			if(M.current == user)
				to_chat(M.current, "<span class='shadowling'><i>You project this power to the rest of the shadowlings.</i></span>")
			else
				to_chat(M.current, "<span class='shadowling'><b>[user.real_name] has coalesced the strength of the thralls. You can draw upon it at any time to ascend. (Shadowling Evolution Tab)</b></span>")//Tells all the other shadowlings


/obj/effect/proc_holder/spell/self/shadowling_blindness_smoke
...
/obj/effect/proc_holder/spell/self/shadowling_blindness_smoke/can_cast(mob/living/user = usr, charge_check = TRUE, show_message = FALSE)
	if(user.incorporeal_move == INCORPOREAL_MOVE_BASIC)
		return FALSE
	. = ..()

/obj/effect/proc_holder/spell/self/shadowling_blindness_smoke/cast(list/targets, mob/user = usr) //Extremely hacky
	if(!shadowling_check(user))
		revert_cast(user)
		return
	...
	chem_smoke.set_up(reagents_list, 4, user.loc, TRUE)
	chem_smoke.start()
...
/datum/reagent/shadowling_blindness_smoke/on_mob_life(mob/living/carbon/M)
	...
	if(!is_shadow_or_thrall(M))
		...
		update_flags |= M.heal_overall_damage(10, 10, updating_health = FALSE)
		update_flags |= M.adjustOxyLoss(-10, FALSE)
		update_flags |= M.adjustToxLoss(-10, FALSE)
	return ..() | update_flags


/obj/effect/proc_holder/spell/aoe_turf/shadowling_screech
	name = "Sonic Screech"
...
/obj/effect/proc_holder/spell/aoe_turf/shadowling_screech/can_cast(mob/living/user = usr, charge_check = TRUE, show_message = FALSE)
	if(user.incorporeal_move == INCORPOREAL_MOVE_BASIC)
		return FALSE
	. = ..()

/obj/effect/proc_holder/spell/aoe_turf/shadowling_screech/cast(list/targets, mob/user = usr)
	if(!shadowling_check(user))
		revert_cast(user)
		return
	...
	for(var/turf/turf in targets)
		for(var/mob/target in turf.contents)
			...
			if(iscarbon(target))
				...
				var/distance = get_dist(get_turf(c_mob), get_turf(user))
				c_mob.soundbang_act(1, max(200/max(1,distance), 60), rand(0, 5))
				...
				do_sparks(5, 1, robot)
				robot.DefaultCombatKnockdown(120)

		for(var/obj/structure/window/window in turf.contents)
			window.take_damage(rand(80, 100))


/obj/effect/proc_holder/spell/pointed/shadowling_null_charge
	name = "Null Charge"
...
/obj/effect/proc_holder/spell/pointed/shadowling_null_charge/can_cast(mob/living/user = usr, charge_check = TRUE, show_message = FALSE)
	if(user.incorporeal_move == INCORPOREAL_MOVE_BASIC)
		return FALSE
	. = ..()

/obj/effect/proc_holder/spell/pointed/shadowling_null_charge/can_target(atom/target, mob/user, silent)
	. = ..()
	if(!istype(target, /obj/machinery/power/apc))
		if(!silent)
			to_chat(user, "<span class='warning'>You can't use this ability on [target]!</span>")
		return FALSE
	return TRUE

/obj/effect/proc_holder/spell/pointed/shadowling_null_charge/cast(list/targets, mob/living/carbon/human/user = usr)
	if(!shadowling_check(user))
		revert_cast(user)
		return
	...
	if(!do_after(user, 20 SECONDS, target = target_apc))
		...
		target_apc.overload_lighting()
		target_apc.update_icon()


/obj/effect/proc_holder/spell/pointed/shadowling_revive_thrall
	...
	active_msg = "<span class='notice'>You start focusing your powers on mending wounds of allies. <B>Left-click to cast at a target!</B></span>"
	deactive_msg = "<span class='notice'>Your mind relaxes.</span>"
	//need_active_overlay = TRUE

/obj/effect/proc_holder/spell/pointed/shadowling_revive_thrall/can_cast(mob/living/user = usr, charge_check = TRUE, show_message = FALSE)
	if(user.incorporeal_move == INCORPOREAL_MOVE_BASIC)
		return FALSE
	. = ..()

/obj/effect/proc_holder/spell/pointed/shadowling_revive_thrall/can_target(atom/target, mob/user, silent)
	. = ..()
	return is_thrall(target)

/obj/effect/proc_holder/spell/pointed/shadowling_revive_thrall/cast(list/targets, mob/user = usr)
	...
	if(thrall.stat == CONSCIOUS)
		...
		for(var/datum/mind/M in D.ling_team.members)
			...
			var/mob/living/carbon/human/h_mob = M.current
		...
		thrall.set_species(/datum/species/shadowling/lesser)
		thrall.mind.RemoveSpell(/obj/effect/proc_holder/spell/self/shadowling_guise)
		thrall.mind.AddSpell(new /obj/effect/proc_holder/spell/pointed/shadowling_glare(null))
		thrall.mind.AddSpell(new /obj/effect/proc_holder/spell/self/shadowling_shadow_walk(null))
...
/obj/effect/proc_holder/spell/pointed/shadowling_extend_shuttle
	...
	clothes_req = FALSE
	cooldown_min = 60 SECONDS
	active_msg = "<span class='notice'>You start gathering destructive powers to delay the shuttle. <B>Left-click to cast at a target!</B></span>"
	deactive_msg = "<span class='notice'>Your mind relaxes.</span>"
	action_icon = 'icons/mob/actions/actions_shadowling.dmi'
...
/obj/effect/proc_holder/spell/pointed/shadowling_extend_shuttle/can_target(mob/living/carbon/human/target, user, silent)
	. = ..()
	if(!ishuman(target))
		if(!silent)
			to_chat(user, "<span class='warning'>You can't use [target] for this sacrifice!</span>")
		return FALSE
	return !target.stat && !is_shadow_or_thrall(target)

/obj/effect/proc_holder/spell/pointed/shadowling_extend_shuttle/cast(list/targets, mob/user = usr)
	var/mob/living/carbon/human/target = targets[1]
	...
	if(SSshuttle.emergency.mode == SHUTTLE_CALL)
		...
		priority_announce("Крупный системный сбой на борту эвакуационного шаттла. Это увеличит время прибытия примерно на 10 минут, шаттл не может быть отозван.", "Системный сбой.", 'sound/misc/notice1.ogg')
