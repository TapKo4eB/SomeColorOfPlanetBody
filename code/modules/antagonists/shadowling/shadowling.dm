...
Original code was designed for baystation, and moreover due to the nature of
the current codebase the main playable gamemode (dynamic) is the one that
actually doesn't use other game_mode's at all and instead just drops antag roles.

Tellin' you this cuz forcing Shadowlings gamemode through Game-Panel just ain't possible.

Simple explanation 'bout shadowlings at all:
	- Everything that makes player a shadowling is just a spell. Not even an antag role, just spell.
	- User presses the spell: he becomes the shadowling in a traditional sense
~ Slipper

PREFLIGHT CHECKLIST:
	- [] It should work inside Quick-Create Antagonist
	- [] It should work inside Traitor Panel
	- [] It should work via Dynamic Mode
	- [] Dethrall Shadowling Surgery (lmao)
	- [] Shadowling saymode
	- [] Resolve all TODOs
...
/*
	TRAITOR DATUM
*/
/datum/antagonist/shadowling/thrall
	name = "Thrall"

/datum/antagonist/shadowling/thrall/greet()
	...
	to_chat(owner, "<span class='shadowling'><b>The shadowlings are your masters.</b> Serve them above all else and ensure they complete their goals.</span>")
	...
	to_chat(owner, "<span class='shadowling'>You may communicate with your allies by speaking in the Shadowling Hivemind (:8).</span>")

/datum/antagonist/shadowling/thrall/grant_powers()
	if(!owner)
		return
	var/datum/mind/M = owner
	M.AddSpell(new /obj/effect/proc_holder/spell/self/shadowling_guise(null))
	M.AddSpell(new /obj/effect/proc_holder/spell/self/shadowling_vision/thrall(null))

/datum/antagonist/shadowling/thrall/apply_innate_effects()
	remove_clownmut()
	grant_powers()

/datum/antagonist/shadowling
	name = "Shadowling"
	roundend_category = "Shadowlings" //just in case
	antagpanel_category = "Shadowling"
	job_rank = ROLE_SHADOWLING
	antag_moodlet = /datum/mood_event/focused
	threat = 10 // TODO
	soft_antag = FALSE
	var/datum/team/shadowlings/ling_team

/datum/antagonist/shadowling/apply_innate_effects()
	. = ..()
	remove_clownmut()
	grant_powers()
	//ling_team.shadows += owner
	//owner.current.log_message("has been converted to the revolution!", LOG_ATTACK, color="red") //TODO

/datum/antagonist/shadowling/on_removal()
	//ling_team.shadows -= owner
	. = ..()

/datum/antagonist/shadowling/proc/grant_powers()
	if(!owner)
		return
	var/datum/mind/ling_mind = owner
	ling_mind.AddSpell(new /obj/effect/proc_holder/spell/self/shadowling_hatch(null))
/*
	spawn(0)
		// TODO: NO hivemind langugages in TG, gonna need an additional ability/saymode.
		shadow_mind.current.add_language("Shadowling Hivemind")
		// TODO: add hud icons
		update_shadow_icons_added(shadow_mind)
*/
/datum/antagonist/shadowling/proc/remove_clownmut()
	if (owner)
		var/mob/living/carbon/human/H = owner.current
		if(istype(H) && owner.assigned_role == "Clown")
			to_chat(H, "Your alien nature has allowed you to overcome your clownishness.")
			H.dna.remove_mutation(CLOWNMUT)

/datum/antagonist/shadowling/greet() // TODO: look if it looks like shit
	to_chat(owner, "<b>Currently, you are disguised as an employee aboard [world.name].</b>")
	to_chat(owner, "<b>In your limited state, you have two abilities: Hatch and Shadowling Hivemind (:8).</b>")
...
/datum/antagonist/shadowling/create_team(datum/team/shadowlings/new_team)
	if(!new_team)
		...
		for(var/datum/antagonist/shadowling/H in GLOB.antagonists)
			if(!H.owner)
				continue
			if(H.ling_team)
				ling_team = H.ling_team
				return
		ling_team = new /datum/team/shadowlings
		//ling_team.update_objectives()
		return
	if(!istype(new_team))
...
/datum/team/shadowlings/New(starting_members)
	. = ..()
	warning_threshold = round(0.66 * required_thralls)
	thrall_ratio = required_thralls / 15

/datum/team/shadowlings/proc/add_thrall(datum/mind/new_thrall_mind)
	if(!istype(new_thrall_mind))
		return FALSE
	if(jobban_isbanned(new_thrall_mind.current, ROLE_SHADOWLING_THRALL)) // it's gonna be amusing if it's ever be used once
	//TODO: change to actual antag code
		var/mob/living/carbon/human/H = new_thrall_mind.current
		H.gib()
		return
	//shadowling_thralls += new_thrall_mind
	new_thrall_mind.add_antag_datum(/datum/antagonist/shadowling/thrall, src)
	//new_thrall_mind.special_role = ROLE_SHADOWLING_THRALL
	...
	if(!victory_warning_announced && (get_thralls() >= warning_threshold))//are the slings very close to winning?
		victory_warning_announced = TRUE	//then let's give the station a warning
		priority_announce("Сканерами дальнего действия обнаружена большая концентрация психической блюспейс-энергии. Событие вознесения тенеморфов неизбежно. Предотвратите его любой ценой!", "Отдел Центрального Командования по делам высших измерений.", ANNOUNCER_SPANOMALIES)

/datum/team/shadowlings/proc/get_thralls()
	var/count = 0
	for(var/datum/mind/M in members)
		if(M.has_antag_datum(/datum/antagonist/shadowling/thrall))
			count++
	return count

/proc/is_shadow_or_thrall(mob/M)
	return M && istype(M) && M.mind && ((M.mind.has_antag_datum(/datum/antagonist/shadowling/thrall)) || (M.mind.has_antag_datum(/datum/antagonist/shadowling)))

/proc/is_shadow(mob/M)
	return M && istype(M) && M.mind && M.mind.has_antag_datum(/datum/antagonist/shadowling)

/proc/is_thrall(mob/M)
	return M && istype(M) && M.mind && M.mind.has_antag_datum(/datum/antagonist/shadowling/thrall)


// below ported code needing cleanup

// Support variables, you can't get away without them.
/*
/datum/game_mode
	var/list/datum/mind/shadows = list()
	var/list/datum/mind/shadowling_thralls = list()
...
/datum/game_mode/shadowling/pre_setup()
	...
	while(shadowlings)
		...
		modePlayer += shadow
		shadow.special_role = ROLE_SHADOWLING
...
/datum/game_mode/proc/finalize_shadowling(datum/mind/shadow_mind)
	...
	spawn(0)
		// TODO: NO hivemind langugages in TG, gonna need an additional spell to comm. sad thing is that's actions have no cooldown
		shadow_mind.current.add_language("Shadowling Hivemind")
		update_shadow_icons_added(shadow_mind)
...
/datum/game_mode/proc/add_thrall(datum/mind/new_thrall_mind)
	...
	if(!(new_thrall_mind in shadowling_thralls))
		shadowling_thralls += new_thrall_mind
		new_thrall_mind.special_role = SPECIAL_ROLE_SHADOWLING_THRALL // TODO: think about if it even needed at all
		update_shadow_icons_added(new_thrall_mind)
		add_conversion_logs(new_thrall_mind.current, "Became a Shadow thrall")
		new_thrall_mind.current.add_language("Shadowling Hivemind")
		//If you add spells to thrall, be sure to remove them on dethrallize
		...
		to_chat(new_thrall_mind.current, "<span class='shadowling'>You may communicate with your allies by speaking in the Shadowling Hivemind (:8).</span>") //TODO
...
/datum/game_mode/shadowling/check_finished()
	...
	for(var/datum/mind/shadow in shadows) //but what if shadowling was not kill?
		...
		if(shadow.current.stat == DEAD)
			continue
		shadows_alive++
		if(shadow.special_role == ROLE_SHADOWLING && config.shadowling_max_age)
